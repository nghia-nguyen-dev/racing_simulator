...
async function onPageLoad(): void {
	try {
		getTracks().then((tracks: Track[]): void => {
			const html = renderTrackCards(tracks);
			renderAt("#tracks", html);
		});

		getRacers().then((racers: Racer[]): void => {
...
async function handleCreateRace() {
	try {
		...
		renderAt(
			"#race",
			renderRaceStartView(store.track.name, store.player_id)
		);
		...
		const race = await createRace(
			parseInt(store.player_id),
			parseInt(store.track.id)
		);
...
function raceProgress(positions: RacerPos[]) {
	let userPlayer = positions.find(
		(pos: RacerPos) => pos.id === parseInt(store.player_id)
	);
...
function createRace(player_id: number, track_id: number): Promise<Race> {
